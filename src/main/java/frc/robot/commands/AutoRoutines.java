...
public class AutoRoutines {
        ...
        public void configure() {
                ...
                //autoChooser.addRoutine("CENTER-LEFT: Ballpit Mid → LEFT Shoot", this::CL_Bm_Ls);
                //autoChooser.addRoutine("CENTER-LEFT: Ballpit Long → LEFT Shoot", this::CL_Bl_Ls);
                //autoChooser.addRoutine("CENTER-LEFT: Ballpit Long → RIGHT Shoot", this::CL_Bl_Rs);
                
                // === CENTER-RIGHT ROUTINES ===
                //autoChooser.addRoutine("CENTER-RIGHT: Ballpit Mid → RIGHT Shoot", this::CR_Bm_Rs);
                //autoChooser.addRoutine("CENTER-RIGHT: Ballpit Long → RIGHT Shoot", this::CR_Bl_Rs);
                //autoChooser.addRoutine("CENTER-RIGHT: Ballpit Mid → LEFT Shoot", this::CR_Bm_Ls);
                //autoChooser.addRoutine("CENTER-RIGHT: Ballpit Long → LEFT Shoot", this::CR_Bl_Ls);
        ...
        // ========== HELPER METHOD ==========
        private void setupAutoRoutine(AutoRoutine routine, AutoTrajectory autoA, AutoTrajectory autoB, AutoTrajectory autoC,
                        double shooterRPM, double hoodPosition) {
                routine.active().onTrue(
                                Commands.sequence(
                                                Commands.runOnce(() -> {
                                                        m_field.getObject("traj1").setPoses(autoA.getRawTrajectory().getPoses());
                                                        m_field.getObject("traj2").setPoses(autoB.getRawTrajectory().getPoses());
                                                        m_field.getObject("traj3").setPoses(autoC.getRawTrajectory().getPoses());
                                                }),
                                                autoA.resetOdometry(),
                                                autoA.cmd()));
