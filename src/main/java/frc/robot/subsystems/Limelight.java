...
public class Limelight extends SubsystemBase {
    ...
    private final StructPublisher<Pose2d> posePubliser;

    public Limelight(String name) {
        this.name = name;
        this.telemetryTable = NetworkTableInstance.getDefault().getTable("SmartDashboard/" + name);
        this.posePubliser = telemetryTable.getStructTopic("Estimated Robot Pose", Pose2d.struct).publish();
    }
    ...
    public Optional<Measurement> getMeasurement(Pose2d currentRobotPose) {
        ...
        final Matrix<N3, N1> standarDeviations = VecBuilder.fill(0.1, 0.1, 10.0);

        posePubliser.set(poseEstimate_MegaTag2.pose);

        return Optional.of(new Measurement(poseEstimate_MegaTag2, standarDeviations));
    }
